...
class MenuCharacterEditorState extends MusicBeatState
{
	...
	function addCharacterUI() {
		...
		scaleStepper = new FlxUINumericStepper(140, imageInputText.y, 0.05, 0.5, 0.1, 30, 2);
	...
	override function update(elapsed:Float) {
		...
		holdTimer = new FlxTimer().start(0.4, function(tmr:FlxTimer)
		{
			...
			if(FlxG.keys.pressed.LEFT && rapidMove) {
			...
			if(FlxG.keys.pressed.RIGHT && rapidMove) {
				characterFile.position[0] -= shiftMult;
				updateOffset();
				holdTime = 0;
			}
			...
			if(FlxG.keys.pressed.UP && rapidMove) {
			...
			if(FlxG.keys.pressed.DOWN && rapidMove) {
				characterFile.position[1] -= shiftMult;
				updateOffset();
				holdTime = 0;
			}
