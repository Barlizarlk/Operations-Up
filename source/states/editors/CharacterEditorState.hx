...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		\nHold Arrow Keys to Rapidly Move
		\nHold Alt to Move The Character Camera\n".split('\n');
	...
	override function update(elapsed:Float)
	{
		...
		if(!charDropDown.dropPanel.visible) {
			...
			if(char.animationsArray.length > 0) {
				...
				controlArray= [FlxG.keys.justPressed.LEFT, FlxG.keys.justPressed.RIGHT, FlxG.keys.justPressed.UP, FlxG.keys.justPressed.DOWN];

		if(FlxG.keys.justPressed.LEFT || FlxG.keys.justPressed.DOWN || FlxG.keys.justPressed.UP || FlxG.keys.justPressed.RIGHT) {
		holdTimer = new FlxTimer().start(0.4, function(tmr:FlxTimer)
		{
			rapidMove = true;
		});
		}

    		if (FlxG.keys.justReleased.LEFT || FlxG.keys.justReleased.DOWN || FlxG.keys.justReleased.UP || FlxG.keys.justReleased.RIGHT) {
			rapidMove = false;
			holdTimer.cancel();
		}
   		...
   		if (FlxG.keys.pressed.ALT) 
			...
			camMove = false;
						...
						if (rapidMove && camMove && FlxG.keys.pressed.LEFT){
						positionCameraXStepper.value += 1 * moveMultiplier;
						char.cameraPosition[0] = positionCameraXStepper.value;
						updatePointerPos();
						}
						if (camMove && FlxG.keys.justPressed.LEFT){
						positionCameraXStepper.value += 1 * moveMultiplier;
						char.cameraPosition[0] = positionCameraXStepper.value;
						updatePointerPos();
					}

						if (rapidMove && camMove && FlxG.keys.pressed.RIGHT){
						positionCameraXStepper.value -= 1 * moveMultiplier;
						char.cameraPosition[0] = positionCameraXStepper.value;
						updatePointerPos();
						}
						if (camMove && FlxG.keys.justPressed.RIGHT){
						positionCameraXStepper.value -= 1 * moveMultiplier;
						char.cameraPosition[0] = positionCameraXStepper.value;
						updatePointerPos();
					}

						if (rapidMove && camMove && FlxG.keys.pressed.UP){
						positionCameraYStepper.value -= 1 * moveMultiplier;
						char.cameraPosition[1] = positionCameraYStepper.value;
						updatePointerPos();
						}
						if (camMove && FlxG.keys.justPressed.UP){
						positionCameraYStepper.value -= 1 * moveMultiplier;
						char.cameraPosition[1] = positionCameraYStepper.value;
						updatePointerPos();
					}

						if (rapidMove && camMove && FlxG.keys.pressed.DOWN){
						positionCameraYStepper.value += 1 * moveMultiplier;
						char.cameraPosition[1] = positionCameraYStepper.value;
						updatePointerPos();
						}
						if (camMove && FlxG.keys.justPressed.DOWN){
						positionCameraYStepper.value += 1 * moveMultiplier;
						char.cameraPosition[1] = positionCameraYStepper.value;
						updatePointerPos();
					}
