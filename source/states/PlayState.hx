...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['E', 0.35], //From 0% to 19%
		['D-', 0.4], //From 20% to 39%
		['D', 0.45], //From 20% to 39%
		['D+', 0.5], //From 40% to 49%
		['C-', 0.55], //From 40% to 49%
		['C', 0.6], //From 50% to 59%
		['C+', 0.65], //From 60% to 68%
		['B-', 0.7], //69%
		['B', 0.75], //69%
		['B+', 0.8], //From 70% to 79%
		['A-', 0.85], //From 70% to 79%
		['A', 0.9], //From 80% to 89%
		['A+', 0.93], //From 80% to 89%
		['S-', 0.95], //From 80% to 89%
		['S', 0.96], //From 80% to 89%
		['SS-', 0.98], //From 80% to 89%
		['SS', 1], //From 90% to 99%
	...
	public var noteBeBigY:FlxTween;
	public var noteGoSmallY:FlxTween;
	public var noteBeBigX:FlxTween;
	public var noteGoSmallX:FlxTween;
	public var noteGoSmallTwoY:FlxTween;
	public var noteGoSmallTwoX:FlxTween;
	...
	public var vocalOpponent:FlxSound;
	public var vocalBf:FlxSound;
	public static var bfMuted:Bool = false;
	...
	public var health:Float = ClientPrefs.data.StartHealth;
	public var combo:Int = 0;

	var lives:Int = ClientPrefs.data.Lives;
	...
	override public function create()
	{
		...
		wiggleShit.effectType = shaders.WiggleEffect.WiggleEffectType.DREAMY;
		...
		// le wiggle 2
		var wiggleShit2:shaders.WiggleEffect = new shaders.WiggleEffect();
		wiggleShit2.waveAmplitude = 0.04;
		wiggleShit2.effectType = shaders.WiggleEffect.WiggleEffectType.HEAT_WAVE_VERTICAL;
		...
		if (ClientPrefs.data.wavySustains)
		camSus.setFilters([susWiggle]); // only enable it for snake notes

		if (ClientPrefs.data.wavyHud)
		camHUD.setFilters([susWiggle]);

		if (ClientPrefs.data.wavyGame)
		camGame.setFilters([susWiggle]);
		...
		if (ClientPrefs.data.timeBarType != 'Disabled' && !ClientPrefs.data.psychHud) // I dont wanna talk about this code :(
		{
			songPosBG = new FlxSprite(0, 10).loadGraphic(Paths.image('healthBar2', 'shared'));
		...
		healthBar = new HealthBar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.11), 'healthBar', function() return health, 0, ClientPrefs.data.MaxHealth);
		...
		if (ClientPrefs.data.psychHud)
		iconP1.y = healthBar.y - 75;
		else
		iconP1.y = healthBar.y - (iconP1.height / 2);

		if (!ClientPrefs.data.hideHud && ClientPrefs.data.hpIcons)
		iconP1.visible = true;
		else
		...
		rateTxt = new FlxText(npsTxt.x, npsTxt.y - 26, 0, "", 20);
		if (ClientPrefs.data.downScroll)
			rateTxt.y = npsTxt.y + 26;
		rateTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, RIGHT);
		rateTxt.setBorderStyle(OUTLINE, 0xFF000000, 3, 1);
		rateTxt.scrollFactor.set();
		if (!ClientPrefs.data.hideHud) {
			rateTxt.visible = false;
		} else {
			rateTxt.visible = false;
		}
		add(rateTxt);

		livesTxt = new FlxText(rateTxt.x, rateTxt.y - 26, 0, "", 20);
		if (ClientPrefs.data.downScroll)
			livesTxt.y = rateTxt.y + 26;
		livesTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, RIGHT);
		livesTxt.setBorderStyle(OUTLINE, 0xFF000000, 3, 1);
		livesTxt.scrollFactor.set();
		if (!ClientPrefs.data.hideHud && lives != 1) {
		...
		rateTxt.text = "Rating: " + ratingName;
		...
		add(BlindingBG);
			...
			LightsOutBG.alpha = ClientPrefs.data.brightness / 100 * -1;
			BlindingBG.alpha = ClientPrefs.data.brightness / 100;
		...
		super.create();
						...
						arrowLaneRando = FlxG.random.int(0, 3);
						arrowLaneRando2 = FlxG.random.int(0, 3);
						arrowLaneRando3 = FlxG.random.int(0, 3);
		Paths.clearUnusedMemory();
		
		CustomFadeTransition.nextCamera = camOther;
		if(eventNotes.length < 1) checkEventNote();

		if (ClientPrefs.data.Mirror)
		{
		...
		if (ClientPrefs.data.UpsideDown)
		{
	...
	function set_playbackRate(value:Float):Float
	{
		if(generatedMusic)
		{
			...
			if(vocalOpponent != null) vocalOpponent.pitch = value;
			if(vocalBf != null) vocalBf.pitch = value;
	...
	public function reloadHealthBarColors() {
				...
				if (ClientPrefs.data.coloredHealth) {
		healthBar.setColors(FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]),
			FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]));
			}
			else
			{
			healthBar.setColors(FlxColor.fromRGB(225, 0, 0),
			FlxColor.fromRGB(50, 205, 50));
			}
	}
	...
	public function startCountdown()
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				{
					case 0:
						FlxG.sound.play(Paths.sound('intro3' + introSoundsSuffix), ClientPrefs.data.soundVolume / 100 - 0.4);
						tick = THREE;
					case 1:
						countdownReady = createCountdownSprite(introAlts[0], antialias);
						FlxG.sound.play(Paths.sound('intro2' + introSoundsSuffix), ClientPrefs.data.soundVolume / 100 - 0.4);
						tick = TWO;
					case 2:
						countdownSet = createCountdownSprite(introAlts[1], antialias);
						FlxG.sound.play(Paths.sound('intro1' + introSoundsSuffix), ClientPrefs.data.soundVolume / 100 - 0.4);
						tick = ONE;
					case 3:
						countdownGo = createCountdownSprite(introAlts[2], antialias);
						FlxG.sound.play(Paths.sound('introGo' + introSoundsSuffix), ClientPrefs.data.soundVolume / 100 - 0.4);
						tick = GO;
					case 4:
						tick = START;
				}
	...
	public function setSongTime(time:Float)
	{
		...
		if (Conductor.songPosition <= vocals.length)
		{
			...
			else if (Conductor.songPosition <= vocalBf.length) {
			vocalBf.time = Conductor.songPosition;
			vocalBf.pitch = playbackRate;
		}

			else if (Conductor.songPosition <= vocalOpponent.length) {
			vocalOpponent.time = Conductor.songPosition;
			vocalOpponent.pitch = playbackRate;
		}
		vocals.play();
		vocalOpponent.play();
		vocalBf.play();
	...
	private function generateSong(dataPath:String):Void
	{
		...
		vocals = new FlxSound();
		vocalOpponent = new FlxSound();
		vocalBf = new FlxSound();

		if (songData.needsVoices) {
		vocals.loadEmbedded(Paths.voices(songData.song));
		vocalOpponent.loadEmbedded(Paths.voiceOpponent(songData.song));
		vocalBf.loadEmbedded(Paths.voiceBf(songData.song));
		}

		vocals.pitch = playbackRate;
		vocals.volume = ClientPrefs.data.vocalsVolume / 100;
		FlxG.sound.list.add(vocals);

		vocalOpponent.pitch = playbackRate;
		vocalOpponent.volume = ClientPrefs.data.vocalsVolume / 100;
		FlxG.sound.list.add(vocalOpponent);

		vocalBf.pitch = playbackRate;
		vocalBf.volume = ClientPrefs.data.vocalsVolume / 100;
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				swagNote.scrollFactor.set();
					...
					swagNote.scale.x *= ClientPrefs.data.wideNotes / 100 + 1;

					swagNote.scale.y *= ClientPrefs.data.tallNotes / 100 + 1;

					if (ClientPrefs.data.InvisibleNotes)
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...4)
		{
			...
			strumLineNotes.add(babyArrow);
				...
				if (ClientPrefs.data.FlippedNotes)
				babyArrow.angle = 180;
			else
				babyArrow.angle = 0;
	...
	override public function update(elapsed:Float)
	{
		...
		if(!inCutscene && !paused) {
			FlxG.camera.followLerp = FlxMath.bound(elapsed * 2.4 * cameraSpeed * ClientPrefs.data.cameraSpeed * playbackRate / (FlxG.updateFramerate / 60), 0, 1);
		...
		if(bfMuted && ClientPrefs.data.muteVocalsOnMiss) {
		vocalBf.volume = 0;
		}
		else {
		vocalBf.volume = ClientPrefs.data.vocalsVolume / 100;
				...
				FlxG.cameras.shake(ClientPrefs.data.Earthquake / 2000, 0.2);

				health += ClientPrefs.data.Love / 600000;

				health -= ClientPrefs.data.Fright / 700000;

				FlxG.camera.angle += Math.sin(Conductor.songPosition * Conductor.bpm / 100 / 500) * (0.008 * ClientPrefs.data.Seasick);
				camHUD.angle += Math.cos(Conductor.songPosition * Conductor.bpm / 100 / 500) * (0.008 * ClientPrefs.data.Seasick);
				camNOTES.angle += Math.cos(Conductor.songPosition * Conductor.bpm / 100 / 500) * (0.008 * ClientPrefs.data.Seasick);
				camSus.angle += Math.cos(Conductor.songPosition * Conductor.bpm / 100 / 500) * (0.008 * ClientPrefs.data.Seasick);

				FlxG.camera.angle += 0.01 * ClientPrefs.data.CameraSpin;
				camHUD.angle -= 0.01 * ClientPrefs.data.CameraSpin;
				camNOTES.angle -= 0.01 * ClientPrefs.data.CameraSpin;
				camSus.angle -= 0.01 * ClientPrefs.data.CameraSpin;

					// camNOTES.x += (ClientPrefs.data.SnakeNotes * 0.025) * Math.sin(Conductor.songPosition / 300);
					camNOTES.y += (ClientPrefs.data.DrunkNotes * 0.025) * Math.sin(Conductor.songPosition / 300);
					// camSus.x += (ClientPrefs.data.SnakeNotes * 0.025) * Math.sin(Conductor.songPosition / 300);
					camSus.y += (ClientPrefs.data.DrunkNotes * 0.025) * Math.sin(Conductor.songPosition / 300);



		wiggleShit.waveAmplitude = FlxMath.lerp(wiggleShit.waveAmplitude, 0, 0.035 / (ClientPrefs.data.framerate / 60));
		wiggleShit.waveFrequency = FlxMath.lerp(wiggleShit.waveFrequency, 0, 0.035 / (ClientPrefs.data.framerate / 60));
		...
		if (currentFrames == ClientPrefs.data.framerate && !ClientPrefs.data.hideHud)
		{
			...
			currentFrames++;
		else if (ClientPrefs.data.hideHud)
			nps = 0; // shut up
		...
		if (health > ClientPrefs.data.MaxHealth) health = ClientPrefs.data.MaxHealth;
		...
		if (camZooming)
		{
			...
			camNOTES.zoom = FlxMath.lerp(1, camNOTES.zoom, FlxMath.bound(1 - (elapsed * 3.125 * camZoomingDecay * playbackRate), 0, 1));
			camHUD.zoom = FlxMath.lerp(1, camHUD.zoom, FlxMath.bound(1 - (elapsed * 3.125 * camZoomingDecay * playbackRate), 0, 1));
			camSus.zoom = FlxMath.lerp(1, camSus.zoom, FlxMath.bound(1 - (elapsed * 3.125 * camZoomingDecay * playbackRate), 0, 1));
		}
				...
				if(notes.length > 0)
				{
					if(startedCountdown)
					{
						...
						notes.forEachAlive(function(daNote:Note)
						{
							...
							daNote.y += 0.25 * FlxG.random.int(Std.int(ClientPrefs.data.HyperNotes * -1), Std.int(ClientPrefs.data.HyperNotes));
							daNote.x += 0.25 * FlxG.random.int(Std.int(ClientPrefs.data.HyperNotes * -1), Std.int(ClientPrefs.data.HyperNotes));

							daNote.x += (ClientPrefs.data.SnakeNotes * 2.025) * Math.sin(Conductor.songPosition / 300);
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		...
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && !ClientPrefs.data.Practice && lives != 1 || isSurvival && survivalTimer <= 0 && Conductor.songPosition > 250)
		{
			FlxG.camera.flash(0xFFFF0000, 0.3 * SONG.bpm / 100);
			lives -= 1;
			songMisses = 0;
			health = ClientPrefs.data.MaxHealth / 2;
			FlxG.sound.play(Paths.sound('missnote2', 'shared'), ClientPrefs.data.soundVolume / 100 - 0.2);
			callOnScripts('onLifeLost');
		}



		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && !ClientPrefs.data.Practice && lives == 1 && !isSurvival || isSurvival && survivalTimer <= 0)
		{
		...
		switch(eventName) {
			...
			case 'Add Camera Zoom':
				if(ClientPrefs.data.camZooms && FlxG.camera.zoom < 1.35) {
					...
					camSus.zoom += flValue2;
					camNOTES.zoom += flValue2;
				}
			...
			case 'Screen Shake':
				...
				var targetsArray:Array<FlxCamera> = [camGame, camHUD, camSus, camNOTES];
	...
	public function moveCamera(isDad:Bool)
	{
		if(isDad)
		{
			...
			FlxMath.lerp(camFollow.x, dad.cameraPosition[0] + opponentCameraOffset[0], (camLerp * ClientPrefs.data.cameraSpeed) / (ClientPrefs.data.framerate));
			FlxMath.lerp(camFollow.y, dad.cameraPosition[1] + opponentCameraOffset[1], (camLerp * ClientPrefs.data.cameraSpeed) / (ClientPrefs.data.framerate));
			tweenCamIn();
		}
		else
		{
			...
			FlxMath.lerp(camFollow.x, boyfriend.cameraPosition[0] + boyfriendCameraOffset[0], (camLerp * ClientPrefs.data.cameraSpeed) / (ClientPrefs.data.framerate));
			FlxMath.lerp(camFollow.y, boyfriend.cameraPosition[1] + boyfriendCameraOffset[1], (camLerp * ClientPrefs.data.cameraSpeed) / (ClientPrefs.data.framerate));
	...
	public function endSong()
	{
		...
		if(!startingSong) {
			notes.forEach(function(daNote:Note) {
				if(daNote.strumTime < songLength - Conductor.safeZoneOffset && !ClientPrefs.data.Practice) {
					health -= 0.05 * healthLoss * ClientPrefs.data.HPLoss;
				}
			});
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if(daRating.noteSplash && !note.noteSplashData.disabled && ClientPrefs.data.noteSplashesVisible)
		...
		rating.angle = FlxG.random.int(-5, 5); // rate angle init
	...
	private function keyPressed(key:Int)
	{
		if (!cpuControlled && startedCountdown && !paused && key > -1)
		{
			...
			if(strumsBlocked[key] != true && spr != null && spr.animation.curAnim.name != 'confirm' && ClientPrefs.data.noteGlow)
			{
				if (!PlayState.isPixelStage) {
				...
				noteGoSmallTwoY = FlxTween.tween(spr, {"scale.y": 0.66}, 0.3, { ease: FlxEase.elasticOut});
				noteGoSmallTwoX = FlxTween.tween(spr, {"scale.x": 0.66}, 0.6, { ease: FlxEase.elasticOut});
				}
	...
	private function keyReleased(key:Int)
	{
		if(!cpuControlled && startedCountdown && !paused)
		{
			...
			if(spr != null)
			{
				...
				if(noteGoSmallTwoY != null) noteGoSmallTwoY.cancel();
				if(noteGoSmallTwoX != null) noteGoSmallTwoX.cancel();
	...
	function noteMissCommon(direction:Int, note:Note = null)
	{
		...
		if (!ClientPrefs.data.Practice) {
		if(note != null) subtract = note.missHealth;
		health -= subtract * healthLoss * ClientPrefs.data.HPLoss;
		}

		if(instakillOnMiss && !ClientPrefs.data.Practice)
		{
			doDeathCheck(true);
		...
		if(char != null && char.hasMissAnimations && ClientPrefs.data.missAnims)
		{
			var suffix:String = '';
		...
		if (ClientPrefs.data.muteVocalsOnMiss)
		vocals.volume = 0;
		bfMuted = true;
	}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		health -= ClientPrefs.data.mustDie / 7000;
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			health += note.hitHealth * healthGain * ClientPrefs.data.HPGain;

									randomChance = FlxG.random.int(0, 10);

									if (randomChance <= ClientPrefs.data.RandomLoss && randomChance != 0)
									health -= note.hitHealth * healthLoss * ClientPrefs.data.HPLoss * 2;
			...
			if(!cpuControlled)
			{
				...
				if(spr != null && ClientPrefs.data.noteGlow) spr.playAnim('confirm', true);
				if (!PlayState.isPixelStage) {
				if (!note.isSustainNote) {
				// if(noteGoSmallY != null) noteGoSmallY.cancel();
				// if(noteGoSmallX != null) noteGoSmallX.cancel();
	...
	public function spawnNoteSplashOnNote(note:Note) {
		if(note != null) {
			...
			if(strum != null)
				...
				if (isSurvival)
				survivalTimer += 1000 * 1 * _survivalVars.addTimeMultiplier;

				FlxTween.color(survivalCountdown, 0.2, FlxColor.GREEN, FlxColor.WHITE, {ease: FlxEase.quadInOut
	...
	override function stepHit()
	{
		if(FlxG.sound.music.time >= -ClientPrefs.data.noteOffset)
		{
			if (Math.abs(FlxG.sound.music.time - (Conductor.songPosition - Conductor.offset)) > (20 * playbackRate )
				|| (SONG.needsVoices && Math.abs(vocals.time - (Conductor.songPosition - Conductor.offset)) > (20 * playbackRate)) && (SONG.needsVoices && Math.abs(vocalBf.time - (Conductor.songPosition - Conductor.offset)) > (20 * playbackRate)) && (SONG.needsVoices && Math.abs(vocalOpponent.time - (Conductor.songPosition - Conductor.offset)) > (20 * playbackRate)) )
			{
				resyncVocals();
			}
		}
	...
	override function sectionHit()
	{
		if (SONG.notes[curSection] != null)
		{
			...
			{
				FlxG.camera.zoom += 0.015 * camZoomingMult * ClientPrefs.data.cameraZoom;
				camHUD.zoom += 0.03 * camZoomingMult * ClientPrefs.data.cameraZoom;
				camSus.zoom += 0.03 * camZoomingMult * ClientPrefs.data.cameraZoom;
				camNOTES.zoom += 0.03 * camZoomingMult * ClientPrefs.data.cameraZoom;
			}
	...
	function strumPlayAnim(isDad:Bool, id:Int, time:Float) {
		...
		if(spr != null && ClientPrefs.data.eNoteGlow) {
	...
	function fullComboUpdate()
	{
		...
		var shits:Int = ratingsData[3].hits;
			...
			if (ClientPrefs.data.Perfect && !ClientPrefs.data.Practice) {
			if (songMisses == 1)
			health -= 100;
			}

			if (ClientPrefs.data.ShittyEnding && !ClientPrefs.data.Practice) {
			if (songMisses == 1 || shits == 1)
			health -= 100;
			}

			if (ClientPrefs.data.BadTrip && !ClientPrefs.data.Practice) {
			if (songMisses == 1 || shits == 1 || bads == 1)
			health -= 100;
			}

			if (ClientPrefs.data.TruePerfect && !ClientPrefs.data.Practice) {
			if (songMisses == 1 || shits == 1 || bads == 1 || goods == 1)
			health -= 100;
			}
		...
		if(songMisses < 1)
		{
			...
			ratingFC = 'SDCB';
			if (songMisses == 10 && ClientPrefs.data.SingleDigits && !ClientPrefs.data.Practice) {
			health -= 100;
			}
	}
