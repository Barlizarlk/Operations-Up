...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		side = new FlxSprite(0).loadGraphic(Paths.image('modsAsset'));
		...
		buttonToggle = new FlxButton(startX, 0, "ON", function()
		{
			...
			updateButtonToggle();
			FlxG.sound.play(Paths.sound('scrollMenu'), ClientPrefs.data.soundVolume / 100 - 0.4);
		});
		...
		if(mods.length < 1) {
			...
			checker.color = defaultColor;
			side.color = defaultColor;
			}
		else
			{
			bg.color = mods[curSelected].color;
			checker.color = mods[curSelected].color;
			side.color = mods[curSelected].color;
			}
